...
function Inbox() {
  ...
  const { inbox, getInbox, fetchingMail } = useAuth();
    ...
    <div className="grid grid-row-12 grid-cols-1 h-full bg-[#181818] text-white">
      ...
      <div className="p-4 row-span-1 bg-[#1E1E1E] shadow-lg">
        ...
        <div className="flex gap-2 items-center">
          <button
            className="bg-[#4A4A4A] text-white px-4 py-2 rounded-lg mt-2 hover:bg-[#5A5A5A] disabled:bg-zinc-800 transition-colors"
            disabled={page === 1}
            onClick={() => {
              if (page > 1) {
                setPage(page - 1);
        ...
        <div className="p-6 rounded-lg flex flex-col gap-2">
          {filteredEmails.length > 0 ? (
            filteredEmails.map((email) => (
              <Link
                to={`/dashboard/inbox/${email.uid}`}
                key={email.uid}
                className="p-3 w-fullcursor-pointer border border-accent rounded-2xl"
              >
                <p className="text-sm text-gray-400">{email.date}</p>
                <h3 className="font-semibold text-gray-100">
                  {decodeSubject(email.subject)}
                </h3>
                <p className="text-gray-300">From: {email.from}</p>
              </Link>
            ))
          ) : fetchingMail ? (
            <p className="text-center text-gray-500">Loading...</p>
